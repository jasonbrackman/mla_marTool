...
class EdgeFlowMirrorCommand(OpenMayaMPx.MPxCommand):
    ...
    def manipulateObject(self, redo):
        if self.task.startswith('geometry'):
            ...
            if redo:
                for i in range(targetPointArray.length()):
                    targetPointArray.set(i, self.newTargetPoints[pointIndex].x,
                                         self.newTargetPoints[pointIndex].y,
                                         self.newTargetPoints[pointIndex].z)
                    pointIndex += 1

            else:
                for i in range(targetPointArray.length()):
                    targetPointArray.set(i, self.oldTargetPoints[pointIndex].x,
                                         self.oldTargetPoints[pointIndex].y,
                                         self.oldTargetPoints[pointIndex].z)
                    pointIndex += 1
        ...
        if self.task == "skinCluster":
            ...
            if redo:
                fnSkinCluster.setWeights(skinPath, self.vtxComponents,
                                         influenceIndices, self.WeightArray, 0)
            else:
                fnSkinCluster.setWeights(skinPath, self.vtxComponents,
                                         influenceIndices, self.oldWeightArray,
                                         0)
    ...
    def doIt(self, args):
        ...
        syntax.addFlag(kDirectionFlag, kDirectionFlagLong,
                       OpenMaya.MSyntax.kDouble)
        syntax.addFlag(kMiddleEdgeFlag, kMiddleEdgeFlagLong,
                       OpenMaya.MSyntax.kString)
        syntax.addFlag(kBaseObjectFlag, kBaseObjectFlagLong,
                       OpenMaya.MSyntax.kString)
        syntax.addFlag(kBaseVertexSpaceFlag, kBaseVertexSpaceFlagLong,
                       OpenMaya.MSyntax.kBoolean)
        syntax.addFlag(kLeftJointsPrefixFlag, kLeftJointsPrefixFlagLong,
                       OpenMaya.MSyntax.kString)
        syntax.addFlag(kRightJointsPrefixFlag, kRightJointsPrefixFlagLong,
                       OpenMaya.MSyntax.kString)
        syntax.addFlag(kOptimizeFlag, kOptimizeFlagLong,
                       OpenMaya.MSyntax.kBoolean)
        ...
        if argData.isFlagSet(kRightJointsPrefixFlag):
            replaceString = argData.flagArgumentString(kRightJointsPrefixFlag,
                                                       0)
        ...
        selectedPoints = OpenMaya.MItMeshVertex(dagPathSelShape,
                                                component)  # problem
        ...
        if doOptimize and len(
                edgeFlowMirrorSavedMapArray) and self.task != 'compute':
            ...
            mapArray, sideArray, newBaseObjectName = self.analizeTopology(
                middleEdge)
        ...
        if self.task == "skinCluster":
            ...
            while not it.isDone():
                ...
                fnSkinCluster.getPathAtIndex(0, skinPath)
                if OpenMaya.MFnDagNode(
                        skinPath.node()).partialPathName() == objectName:
            ...
            if foundSkin:
                ...
                fnSkinCluster.getPathAtIndex(
                    fnSkinCluster.indexForOutputConnection(0), skinPath)
                ...
                for i in range(influenceArray.length()):
                    jointNames.append(
                        OpenMaya.MFnDagNode(influenceArray[i]).name())
                ...
                for i in range(self.infCount):
                    ...
                    if jointNames[i].split(":")[-1].startswith(searchString):
                        searchJointNames.append(
                            nameSpace + jointNames[i].split(':')[-1][
                                        len(searchString):len(jointNames[i])])
                        searchJointIndexes.append(i)
                    elif jointNames[i].split(":")[-1].startswith(replaceString):
                        replaceJointNames.append(
                            nameSpace + jointNames[i].split(':')[-1][
                                        len(replaceString):len(jointNames[i])]);
                ...
                for i in range(len(searchJointNames)):
                    for k in range(len(replaceJointNames)):
                        if searchJointNames[i] == replaceJointNames[k]:
                            jointMapArray[searchJointIndexes[i]] = \
                            replaceJointIndexes[k];
                            jointMapArray[replaceJointIndexes[k]] = \
                            searchJointIndexes[i];
                ...
                fnSkinCluster.getWeights(skinPath, self.vtxComponents,
                                         self.WeightArray, infCountPtr)
                ...
                for i in range(bothIndexes.length()):
                    ...
                    # not middle
                    #
                    if sideArray[index] == direction and mapArray[
                        index] != index:
                        oppositeIndex = allPointsArray[mapArray[bothIndexes[i]]]
                        startIndexA = oppositeIndex * self.infCount

                        for k in range(self.infCount):
                            self.WeightArray[startIndexA + k] = \
                            self.WeightArray[startIndexB + jointMapArray[k]]

                            # middle one
                    #
                    elif (mapArray[index] == index):
                        for k in range(self.infCount):
                            averageWeights[k] = (self.WeightArray[
                                                     startIndexB + k] +
                                                 self.WeightArray[startIndexB +
                                                                  jointMapArray[
                                                                      k]]) * 0.5;
                        for k in range(self.infCount):
                            self.WeightArray[startIndexB + k] = averageWeights[
                                k]
        ...
        if self.task == "geometryFlip" or self.task == "geometryMirror" or self.task == "geometrySymmetry":  # geometry mirror
            ...
            for i in range(targetPoints.length()):
                self.oldTargetPoints.append(
                    OpenMaya.MFloatVector(targetPoints[i].x, targetPoints[i].y,
                                          targetPoints[i].z))
                self.newTargetPoints.append(
                    OpenMaya.MFloatVector(targetPoints[i].x, targetPoints[i].y,
                                          targetPoints[i].z))
            ...
            if mirrorInt != 2:  # if it's not create symmetryshape
                if not doVertexSpace:
                    ...
                    for i in range(len(bothIndexes)):
                        if mirrorInt == 0 or direction == sideArray[
                            bothIndexes[i]]:
                            pointOffset = (
                            targetPoints[bothIndexes[i]] - basePoints[
                                bothIndexes[i]])
                            self.newTargetPoints.set(OpenMaya.MFloatVector(
                                basePoints[
                                    mapArray[bothIndexes[i]]].x - pointOffset.x,
                                basePoints[
                                    mapArray[bothIndexes[i]]].y + pointOffset.y,
                                basePoints[mapArray[
                                    bothIndexes[i]]].z + pointOffset.z),
                                                     mapArray[bothIndexes[i]])
                    ...
                    for i in range(len(bothIndexes)):
                        ...
                        if sideArray[
                            bIndex] == 1:  # dont do the right ones yet, they'll get mirrored
                            continue
                        ...
                        if (basePoints[bIndex] - targetPoints[
                            bIndex]).length() < 0.00001 and (
                            basePoints[mapArray[bIndex]] - targetPoints[
                            mapArray[bIndex]]).length() < 0.00001:
                            doSkipThisOne = True
                        ...
                        for f, face in enumerate(conFaces[bIndex]):
                            ...
                            for vert in vertsOnFaces[face]:
                                if vert != bIndex and vert in conVerts[bIndex]:
                                    for d in range(2):
                                        if twoDots[d] == -1:
                                            twoDots[d] = vert
                                            twoVecs[d] = OpenMaya.MVector(
                                                basePoints[vert] - averagePos)
                                            break
                            ...
                            #
                            if sideArray[bIndex] == 0 and sideArray[
                                twoDots[0]] != 0 and sideArray[twoDots[1]] != 0:
                                continue
                            ...
                            if f > 0:
                                ...
                                if angle0 < angle1:
                                    ...
                                    zNeg.append(twoVecs[1].angle(
                                        firstVecs[1]) > math.pi * 0.5)

                                else:
                                    xId.append(twoDots[1])
                                    zId.append(twoDots[0])
                                    xNeg.append(angle1neg)
                                    zNeg.append(twoVecs[0].angle(
                                        firstVecs[1]) > math.pi * 0.5)
                        ...
                        if mapArray[bothIndexes[i]] != -2:
                            ...
                            if mirrorInt == 0 or direction == sideArray[
                                ...
                                for k in range(idsCount):
                                    ...
                                    baseX += (OpenMaya.MVector(
                                        basePoints[xIds[bIndex][k]] -
                                        basePoints[bIndex])) * xMult
                                    baseZ += (OpenMaya.MVector(
                                        basePoints[zIds[bIndex][k]] -
                                        basePoints[bIndex])) * zMult
                                ...
                                baseMat = createMatrixFromList(
                                    [baseX.x, baseX.y, baseX.z, 0,
                                     baseNormals[bIndex].x,
                                     baseNormals[bIndex].y,
                                     baseNormals[bIndex].z, 0, baseZ.x, baseZ.y,
                                     baseZ.z, 0, basePoints[bIndex].x,
                                     basePoints[bIndex].y, basePoints[bIndex].z,
                                     1])
                                changeWorld = createMatrixFromPos(
                                    targetPoints[bIndex])
                                ...
                                if sideArray[bIndex] == 0:  # middle vertex
                                    ...
                                    #
                                    middleIdVect = OpenMaya.MVector(
                                        basePoints[middleId] - basePoints[
                                            bIndex])
                                    ...
                                    if angleX < angleZ:
                                        centeredLocal = createMatrixFromPos(
                                            OpenMaya.MPoint(changeLocal(3, 0),
                                                            changeLocal(3, 1),
                                                            -changeLocal(3, 2)))
                                    else:
                                        centeredLocal = createMatrixFromPos(
                                    ...
                                    for k in range(idsCount):
                                        ...
                                        targetX += (OpenMaya.MVector(
                                            basePoints[xIds[mappedBIndex][k]] -
                                            basePoints[mappedBIndex])) * xMult
                                        targetZ += (OpenMaya.MVector(
                                            basePoints[zIds[mappedBIndex][k]] -
                                            basePoints[mappedBIndex])) * zMult
                                    ...
                                    targetMat = createMatrixFromList(
                                        [targetX.x, targetX.y, targetX.z, 0,
                                         baseNormals[mappedBIndex].x,
                                         baseNormals[mappedBIndex].y,
                                         baseNormals[mappedBIndex].z, 0,
                                         targetZ.x, targetZ.y, targetZ.z, 0,
                                         basePoints[mappedBIndex].x,
                                         basePoints[mappedBIndex].y,
                                         basePoints[mappedBIndex].z, 1])

                                    changetarget = changeLocal * targetMat

                                self.newTargetPoints.set(
                                    OpenMaya.MFloatVector(changetarget(3, 0),
                                                          changetarget(3, 1),
                                                          changetarget(3, 2)),
                                    mappedBIndex)
                ...
                for i in range(bothIndexes.length()):
                    if sideArray[bothIndexes[i]] == 1 and mapArray[
                        bothIndexes[i]] != -1:
                        targetPoints[bothIndexes[i]].x = targetPoints[
                                                             bothIndexes[
                                                                 i]].x * -1
                ...
                for i in range(bothIndexes.length()):
                    if mapArray[bothIndexes[i]] != -1:
                        ...
                        x = (targetPoints[bothIndexes[i]].x + targetPoints[
                            mapArray[bothIndexes[i]]].x) * 0.5;
                        y = (targetPoints[bothIndexes[i]].y + targetPoints[
                            mapArray[bothIndexes[i]]].y) * 0.5;
                        z = (targetPoints[bothIndexes[i]].z + targetPoints[
                            mapArray[bothIndexes[i]]].z) * 0.5;
                        ...
                        if mapArray[bothIndexes[i]] == bothIndexes[
                            i]:  # middlePoint
                ...
                for i in range(bothIndexes.length()):
                    if mapArray[bothIndexes[i]] != -1:
                        ...
                        self.newTargetPoints.set(OpenMaya.MFloatVector(
                            targetPoints[bothIndexes[i]].x,
                            targetPoints[bothIndexes[i]].y,
                            targetPoints[bothIndexes[i]].z), bothIndexes[i])

                # selecting wrong points:
                fnVtxComp = OpenMaya.MFnSingleIndexedComponent()
                vtxComponent = OpenMaya.MObject();
                self.vtxComponents = fnVtxComp.create(
    ...
    def analizeTopology(self, edge):
        ...
        while True:
            ...
            elif checkedP[l_faceList[0]] == -1 and checkedP[
                l_faceList[1]] != -1:
                l_currentP = l_faceList[0]
            elif checkedP[l_faceList[1]] == -1 and checkedP[
                l_faceList[0]] != -1:
            ...
            elif checkedP[r_faceList[0]] == -1 and checkedP[
                r_faceList[1]] != -1:
                r_currentP = r_faceList[0]
            elif checkedP[r_faceList[1]] == -1 and checkedP[
                r_faceList[0]] != -1:
            ...
            if l_currentE == firstEdge:
                r_edgeVertices0 = scriptUtil.getInt2ArrayItem(r_edgeVertices, 0,
                                                              0)
                r_edgeVertices1 = scriptUtil.getInt2ArrayItem(r_edgeVertices, 0,
                                                              1)
                l_edgeVertices0 = scriptUtil.getInt2ArrayItem(l_edgeVertices, 0,
                                                              0)
                l_edgeVertices1 = scriptUtil.getInt2ArrayItem(l_edgeVertices, 0,
                                                              1)
                ...
                if checkedV[l_edgeVertices0] == -1 and checkedV[
                    r_edgeVertices0] == -1:
                    checkedV[l_edgeVertices0] = r_edgeVertices0
                    checkedV[r_edgeVertices0] = l_edgeVertices0
                if checkedV[l_edgeVertices1] == -1 and checkedV[
                    r_edgeVertices1] == -1:
                    checkedV[l_edgeVertices1] = r_edgeVertices1
                    checkedV[r_edgeVertices1] = l_edgeVertices1
                if checkedV[l_edgeVertices0] == -1 and checkedV[
            ...
            for i in range(l_faceEdgesCount):
                ...
                if checkedE[l_faceEdges[i]] == -1:
                    ...
                    if edgeIter.connectedToEdge(
                            l_faceEdges[i]) and l_currentE != l_faceEdges[i]:

                        fnMesh.getEdgeVertices(l_faceEdges[i],
                                               l_ifCheckedVertices)
                        l_ifCheckedVertice0 = scriptUtil.getInt2ArrayItem(
                            l_ifCheckedVertices, 0, 0)
                        l_ifCheckedVertice1 = scriptUtil.getInt2ArrayItem(
                            l_ifCheckedVertices, 0, 1)
                        ...
                        for k in range(r_faceEdgesCount):
                            edgeIter.setIndex(r_currentE, ptr)
                            if edgeIter.connectedToEdge(
                                    r_faceEdges[k]) and r_currentE != \
                                    r_faceEdges[k]:
                                fnMesh.getEdgeVertices(r_faceEdges[k],
                                                       r_faceEdgeVertices)
                                r_faceEdgeVertice0 = scriptUtil.getInt2ArrayItem(
                                    r_faceEdgeVertices, 0, 0)
                                r_faceEdgeVertice1 = scriptUtil.getInt2ArrayItem(
                                    r_faceEdgeVertices, 0, 1)

                                if r_faceEdgeVertice0 == checkedV[
                                    l_checkedVertex]:
                                    checkedV[
                                        l_nonCheckedVertex] = r_faceEdgeVertice1
                                    checkedV[
                                        r_faceEdgeVertice1] = l_nonCheckedVertex
                                    sideV[l_nonCheckedVertex] = 2
                                    sideV[r_faceEdgeVertice1] = 1
                                    l_edgeQueue.append(l_faceEdges[i])
                                    r_edgeQueue.append(r_faceEdges[k])

                                if r_faceEdgeVertice1 == checkedV[
                                    l_checkedVertex]:
                                    checkedV[
                                        l_nonCheckedVertex] = r_faceEdgeVertice0
                                    checkedV[
                                        r_faceEdgeVertice0] = l_nonCheckedVertex
                                    sideV[l_nonCheckedVertex] = 2
...
def initializePlugin(mobject):
    mplugin = OpenMayaMPx.MFnPlugin(mobject, "Thomas Bittner", "3.4", "Any")
    try:
