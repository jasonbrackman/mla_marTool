...
class BlendShapeManager():
    ...
    def list_controllers(self, *args):
        ...
        controllers_list = [transform for transform in mc.ls(et='transform')
                            if mc.listRelatives(transform, c=True, s=True) is not None
                            and mc.nodeType(mc.listRelatives(transform, c=True, s=True)[0]) == 'nurbsCurve'
                            and 'ctrl' in transform.lower()]
    ...
    def ui(self, *args):
        ...
        # --- Create window
        self.widgets['window'] = mc.window('BlendShapeManager_Window', title='BlendShape Manager 2016 1.0', sizeable=False)
        # --- Create main layout
        ...
        self.widgets['bs_nodes_scrollList'] = mc.textScrollList('bs_nodes_scrollList',
                                                                ...
                                                                selectCommand=self.update_bs_targets_textscrolllist)
    ...
    def get_ui_datas(self, *args):
        ...
        blendshape_node = self.get_selected_textscrolllist_item('bs_nodes_scrollList')[0]
        ...
        def disconnect(*args):  # Creates a callable object
            ...
            # --- Define what to do if delete_attr is False
                ...
                for target in datas[1]:
                    ...
                    set_range = [node for node in all_set_range_nodes if '%s_%s' % (datas[0], target) in node]
    ...
    def write_setup_to_dict(self, *args):
        """
        Save the current blendShape node setup to dict
